...
func swapCSRCsAndHeaderExtension(header *rtp.Header, buf []byte, phase1 bool) {
	...
	if phase1 {
		// Move Header Extension before CSRCs
		...
		// Move CSRCs before Header Extension
...
func encryptCryptextRTP(dst, plaintext []byte, sameBuffer bool, header *rtp.Header,
	encrypt func(dst, plaintext []byte, headerLen int) error,
) error {
	swapCSRCsAndHeaderExtension(header, plaintext, true)
	...
	if err != nil {
		...
		swapCSRCsAndHeaderExtension(header, plaintext, false)
	...
	swapCSRCsAndHeaderExtension(header, dst, false)

	return nil
}

func decryptCryptextRTP(dst, ciphertext []byte, sameBuffer bool, header *rtp.Header, headerLen int,
	decrypt func(dst, ciphertext []byte, headerLen int) error,
) error {
	swapCSRCsAndHeaderExtension(header, ciphertext, true)
	err := decrypt(dst, ciphertext, minSrtpHeaderSize+extensionHeaderSize)
	if err != nil {
		swapCSRCsAndHeaderExtension(header, ciphertext, false)
...
func needsEmptyExtensionHeader(useCryptext bool, header *rtp.Header) bool {
	return useCryptext && len(header.CSRC) > 0 && !header.Extension
}
